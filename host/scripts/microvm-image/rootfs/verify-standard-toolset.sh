...
usage() {
    cat >&2 <<'USAGE'
Usage:
  verify-standard-toolset.sh --live
  verify-standard-toolset.sh --root-dir <prepared-rootfs-dir>
  verify-standard-toolset.sh --rootfs <rootfs.ext4>
  verify-standard-toolset.sh --list-paths

Verifies the commands and Python imports promised by the standard tool-VM
contract. --live executes probes; directory/image modes verify shipped paths.
USAGE
}

fail() {
    echo "standard toolset policy violation: $*" >&2
    exit 1
}
...
import xlsxwriter
PY
    echo "standard toolset policy passed: live root"
}
...
verify_root_dir() {
    ...
    echo "standard toolset policy passed: prepared rootfs"
}
...
verify_rootfs_executable() {
    ...
    local depth=0
    while [ "$depth" -lt 32 ]; do
        stat_output="$(debugfs -R "stat $current" "$image" 2>/dev/null || true)"
...
verify_rootfs() {
    ...
    echo "standard toolset policy passed: rootfs"
}
