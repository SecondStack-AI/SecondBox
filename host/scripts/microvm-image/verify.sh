...
When a public key is supplied, SANDBOX_HOST_MICROVM_PUBLIC_KEY_SHA256 (or the third
...
dir="${1:-}"
pubkey="${2:-${SANDBOX_HOST_MICROVM_PUBLIC_KEY:-}}"
expected_pubkey_sha="${3:-${SANDBOX_HOST_MICROVM_PUBLIC_KEY_SHA256:-}}"
...
if [ -n "$pubkey" ]; then
    ...
    echo "manifest signature not verified: pass a trusted public key or set SANDBOX_HOST_MICROVM_PUBLIC_KEY" >&2
fi
