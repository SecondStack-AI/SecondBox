...
func runLifecycleVerb(ctx context.Context, session cliSession, command string, args []string, output io.Writer, transport *http.Client) error {
	...
	if command == "ls" {
		flags.BoolVar(&all, "all", false, "include other states in the human view (JSON preserves the API page)")
	...
	if command == "ls" {
		...
		if !all {
			items := make([]sb.Sandbox, 0, len(page.Items))
			for _, item := range page.Items {
				if item.State == sb.SandboxStateReady {
