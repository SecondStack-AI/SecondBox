...
func TestCreateGetAndListVerbs(t *testing.T) {
	...
	for _, all := range []bool{false, true} {
		...
		if err := runLifecycleVerb(ctx, session, "ls", args, &output, server.Client()); err != nil {
			t.Fatal(err)
		}
		if strings.Contains(output.String(), "stopped") != all {
			t.Errorf("all=%v output=%s", all, output.String())
		}
	}
}
