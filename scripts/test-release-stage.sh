...
LC_ALL=C "$repo_root/scripts/release-stage.sh" --test-mode --candidate 0.1.0 "$stage_installer_candidate" >/dev/null

jq -e '.candidate == true and .installerQualificationEvidence == {location:"",digest:""}' "$stage_installer_candidate/secondbox-0.1.0-artifact-manifest.json" >/dev/null
[[ ! -e "$stage_installer_candidate/secondbox-0.1.0-installer-qualification-evidence.json" ]] || { echo "installer candidate claimed final qualification evidence" >&2; exit 1; }
candidate_subject="$(go -C "$repo_root" run ./cmd/secondbox-release-tool installer-qualification-subject "$stage_installer_candidate/secondbox-0.1.0-artifact-manifest.json")"
final_subject="$(go -C "$repo_root" run ./cmd/secondbox-release-tool installer-qualification-subject "$stage_one/secondbox-0.1.0-artifact-manifest.json")"
...
for stage in "$stage_one" "$stage_two" "$stage_dictionary_locale"; do
  ...
  jq -e --arg runtime "$runtime_digest" --arg toolchain "$toolchain_digest" '.schemaVersion == "secondbox.release/artifact-manifest/v5" and (.installerTools.reference | startswith("ghcr.io/secondstack-ai/secondbox/installer-tools@sha256:")) and (.installBootstrap.location | endswith("/v0.1.0/install.sh")) and .microvm.runtimeBundle.manifestDigest == $runtime and .microvm.toolchainBundle.manifestDigest == $toolchain and (.qualificationEvidence.location | endswith("/secondbox-0.1.0-qualification-evidence.json")) and (.installerQualificationEvidence.location | endswith("/secondbox-0.1.0-installer-qualification-evidence.json"))' "$stage/secondbox-0.1.0-artifact-manifest.json" >/dev/null
  ...
  jq -e '.schemaVersion == "secondbox.release/installer-qualification-evidence/v1" and .repositoryDirty == false and .suite == "test-installer-qualified" and .passCount == 19 and .wallClockSeconds == 1 and .rebootPassed == true and .host.kvm.present and .host.tun.present and .host.workspaceFilesystem.type == "xfs"' "$stage/secondbox-0.1.0-installer-qualification-evidence.json" >/dev/null
  rg -q 'secondbox-0.1.0-qualification-evidence.json$' "$stage/SHA256SUMS"
  rg -q 'secondbox-0.1.0-installer-qualification-evidence.json$' "$stage/SHA256SUMS"
  rg -q 'install.sh$' "$stage/SHA256SUMS"
  rg -q "version='0.1.0'" "$stage/install.sh"
  rg -q "expected_sha256='$(sha256sum "$stage/secondbox-deploy_0.1.0_linux_amd64" | awk '{print $1}')'" "$stage/install.sh"
  rg -qF '"$binary" install "$@" &' "$stage/install.sh"
