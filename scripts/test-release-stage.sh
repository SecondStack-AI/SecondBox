...
for stage in "$stage_one" "$stage_two" "$stage_dictionary_locale"; do
  ...
  jq -e --arg runtime "$runtime_digest" --arg toolchain "$toolchain_digest" '.schemaVersion == "secondbox.release/artifact-manifest/v6" and .version == "0.7.0" and .runnerProtocol == {minimum:5,maximum:5} and (.installerTools.reference | startswith("ghcr.io/secondstack-ai/secondbox/installer-tools@sha256:")) and (.installBootstrap.location | endswith("/v0.7.0/install.sh")) and .microvm.runtimeBundle.manifestDigest == $runtime and .microvm.toolchainBundle.manifestDigest == $toolchain and (.qualificationEvidence.location | endswith("/secondbox-0.7.0-qualification-evidence.json")) and (.installerQualificationEvidence.location | endswith("/secondbox-0.7.0-installer-qualification-evidence.json"))' "$stage/secondbox-0.7.0-artifact-manifest.json" >/dev/null
  ...
  tar -xOf "$stage/secondbox-0.7.0-go-module.tar.gz" go.mod | rg -q '^retract \[v0\.8\.0, v0\.8\.2\] // Pre-release tags cut before qualification completed \(v0\.8\.0 has an unresolvable module graph; v0\.8\.1 and v0\.8\.2 predate the qualification-driver fixes\); use v0\.8\.3\.$'
  tar -xOf "$stage/secondbox-0.7.0-go-module.tar.gz" go.mod | rg -q '^retract v0\.9\.0 // Tagged before the installer qualification driver accepted the v6 artifact manifest; use v0\.9\.1\.$'
