...
for stage in "$stage_one" "$stage_two" "$stage_dictionary_locale"; do
  ...
  tar -xOf "$stage/secondbox-0.7.0-go-module.tar.gz" go.mod | rg -q '^retract v0\.8\.0 // Tag created from a commit whose module graph is unresolvable downstream; use v0\.8\.1\.$'
