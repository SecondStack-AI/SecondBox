...
for stage in "$stage_one" "$stage_two" "$stage_dictionary_locale"; do
  ...
  tar -xOf "$stage/secondbox-0.7.0-go-module.tar.gz" go.mod | rg -q '^retract \[v0\.8\.0, v0\.8\.1\] // Pre-release tags cut before qualification completed \(v0\.8\.0 has an unresolvable module graph; v0\.8\.1 predates the qualification-driver fixes\); use v0\.8\.2\.$'
