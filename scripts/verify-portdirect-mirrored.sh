...
# The control plane and the privileged Runner each own a copy of the direct Port
...
for source in "$control_plane" "$runner"; do
    if ! grep -q '^package portdirect$' "$source"; then
        echo "SecondBox direct Port handshake copy has no package declaration: $source" >&2
        exit 1
    fi
done

diff -u <(body "$control_plane") <(body "$runner") || {
    echo "SecondBox direct Port handshake copies have diverged: $control_plane vs $runner" >&2
    exit 1
}
