...
Assignments carry provider-neutral `AssetReference` records: artifact identity, immutable digest, architecture, guest protocol generation, and required features.
Generation 5 adds preparation commands before compute assignment and digest-only selected images in assignments.
The unprivileged fetcher uses operator-configured Tenant credentials, and the control plane verifies returned signed component identities before assigning compute.
The Runner verifies local bytes against that assignment without registry access.
Microsandbox and gVisor admission continue to require locally materialized, content-addressed images.

The provider-neutral capability booleans are backend-attested claims with per-backend meaning. For Firecracker, hypervisor readiness is usable KVM and isolation readiness is the jailer. For Microsandbox, hypervisor readiness is a usable KVM or Hypervisor.framework probe. For gVisor, hypervisor readiness attests a successful sentry platform probe (booting and tearing down a trivial sandbox on the systrap platform, with no KVM requirement), isolation readiness attests the pinned runsc identity check, and resource-limit readiness attests cgroup enforcement availability; a gVisor pool provides a userspace-kernel isolation boundary, not hardware virtualization, and operators choose that class explicitly through the pool.
...
An egress-context name is a provider-neutral opaque string matching the single public bound `^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$`, with a maximum of 63 ASCII bytes. Registration advertises a set, not a default or preference. The set is static for one connection; changing Runner configuration requires a restart or reconnect and never remaps an active assignment in place. Registration contains no SecondStack hostname, proxy address, certificate, network range, gateway-address digest, or credential.
...
The context selects a Runner-local logical-gateway mapping. It does not pin gateway addresses or a mapping digest, and addresses may repeat within or across contexts. The selected mapping remains network-policy authorization rather than guest DNS; the Runner does not synthesize logical-gateway records. One external `agent-runner-gateway` instance serves one context and one SecondStack egress proxy, while the gateway and egress-proxy wire protocols remain unchanged. Missing mapping configuration fails the assignment. This release adds no dynamic health discovery, cross-context retry, or live context remapping.
