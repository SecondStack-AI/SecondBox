...
| Flue | [`@flue/runtime` 2.0.1](https://www.npmjs.com/package/@flue/runtime/v/2.0.1), repository tag [`v2.0.1`](https://github.com/withastro/flue/tree/v2.0.1) at commit `a67f00955ac48c14d4b97ffb71962d24e39af84d`, package integrity `sha512-as+rrm8oHLuaLfpSReExwsuzOb1gC0sxQWgz3o+RvJUwHyGcQAcfBcn/R6j8logAq1j+ryfH1WsXf4J7Th9puQ==`, and the official [Sandbox Adapter API](https://flueframework.com/docs/reference/sandbox-api/). SecondBox compiles against that package directly. |
...
The adapter accepts an already initialized SecondBox Sandbox handle and passes its Workspace root to Flue 2.0's `createSandboxSessionEnv`. It rejects an unsupported mkdir or rm option before mutation, preserves real stat fidelity, maps SecondBox terminal outcomes into Flue conventions only at this boundary, and propagates timeout and abort to server-side cancellation. TypeScript compiles and tests against the real Flue package; no vendored compatibility surface remains.
