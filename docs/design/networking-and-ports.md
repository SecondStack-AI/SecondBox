...
Port evidence granularity differs by transport and does so deliberately. The relay transport retains per-frame durability. The direct transport emits the same fixed-shape Runner evidence record at admitted open and at close and retains no per-frame durability; that reduction is the property the direct transport trades away for latency. Neither record can contain a payload byte, a credential, a fencing token, or a Runner address.
