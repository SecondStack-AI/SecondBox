...
| Compromised Runner | Bounded blast radius | Explicit trust pools, credential replacement, drain and fencing, fresh-host recovery, and no platform/database credentials |
...
| Object-store tampering | Durable-byte integrity | Content hashes, size evidence, retention, verified reads, and fenced restore streaming |
...
Security-sensitive mutations produce audit rows. HTTP responses and asynchronous Operations carry request identifiers. Metrics avoid ownership labels. An incident preserves audit and bounded logs, replaces affected credentials, fences affected Runner assignments, removes compromised hosts, and restores only from verified checkpoints on fresh hosts.
