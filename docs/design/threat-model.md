...
Security-sensitive mutations produce audit rows. HTTP responses and asynchronous Operations carry request identifiers. Metrics avoid ownership labels. Relay session rows retain terminal state, correlation, admission replay, compact terminal duplicate evidence, and Port acknowledgement state until session cleanup. Payload frames have a narrower purpose: they remain while delivery or replay can consume them and may be removed earlier. The relay therefore does not promise a transcript after frame cleanup. Direct Port retains only admitted-open and close evidence and never persists payload frames. Reconstructing Port bytes after the applicable frame boundary is impossible; for direct Port that reduction is accepted deliberately in exchange for usable SSH and VS Code Remote-SSH latency. An incident preserves audit and bounded logs, replaces affected credentials, fences affected Runner assignments, and recovers affected Sandboxes only by restoring the same stable Runner identity and its Workspace root from a trusted consistent backup. Without that backup, those Sandboxes and local Snapshots are lost.
