...
Port evidence granularity is transport dependent. The relay transport persists frames while delivery or replay can consume them, then may remove those payload-bearing rows before session cleanup. The session row keeps terminal state, durable sequence/hash projections, acknowledgement state, correlation, and admission replay until its transition-specific deadline; it does not preserve a transcript. The direct transport emits the fixed-shape Runner evidence record at admitted open and at close and retains no per-frame durability. In both transports, payload reconstruction after frame cleanup is outside the forensic boundary. No Port evidence record can contain a payload byte, a credential, a fencing token, or a Runner address.
