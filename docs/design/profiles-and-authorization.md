...
Generation-5 client-selected images currently support only `cold_boot` Firecracker assignments. Do not upgrade a deployment with `snapshot_resume` Profiles until selected-image snapshot materialization exists. A stopped Sandbox pinned to such a Profile cannot restart through a generation-5 control plane.
