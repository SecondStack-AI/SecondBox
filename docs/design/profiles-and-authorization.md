...
The release-owned `agent-compartment` revision 3 adds this permission to its existing ordinary policy, with the same logical Agent gateway and a limit of two simultaneous attributed connections per generation. This is the limit exercised by backend and public scenario qualification. Earlier revisions remain immutable; existing Sandboxes keep their pinned revision. The isolated Profile retains `deny_all` networking and no attributed permission.
