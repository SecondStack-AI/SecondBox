...
Administrative resources cover Projects, ServiceAccounts, APIKeys, Profiles and revisions, RunnerPools, Runners, audit records, and diagnostic bundles. Application resources cover Sandboxes, Operations, Leases, exec sessions, terminal sessions, files, snapshots, artifacts, and port sessions. Administrative runner projections never appear inside application Sandbox responses.
...
Project identity comes from authentication. The request cannot override backend, image, resources, lifecycle, storage, network, timeouts, ports, runner pool, placement, generation, or Instance state.
...
A Lease is explicit, bounded, Project-scoped authority for one Sandbox generation. Acquire, renew, release, and inspect are separate calls. A Lease cannot be renewed after expiry, revocation, drain, or generation change. A stale Lease produces `lease_fenced`, not an authentication error. Profiles determine whether the absence or expiry of Leases contributes to stopping compute.
...
See [Domain and lifecycle](domain-lifecycle.md), [Networking and ports](networking-and-ports.md), [Compatibility policy](compatibility-policy.md), and [API reference comparison](api-reference-comparison.md).
