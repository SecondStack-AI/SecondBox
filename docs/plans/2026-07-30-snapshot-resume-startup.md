...
| Pre-assignment orchestration | Quieter baseline is 320/488 ms; latest loaded-host candidate is 602/1,181 ms, including 262/324 ms of Workspace provisioning | Must fit inside the total target |
| Command delivery and runner admission | Eager-dispatch candidate is 11/34 ms; its read-only claim query is 0/1 ms | 10–25 ms required |
...
| Ready projection and client visibility | Latest 30-arrival measurements are 0/0 ms and 19/31 ms | Must fit inside the total target |
| **Total** | **Quieter retained baseline is 745/929 ms; latest loaded-host eager candidate is 1,015/1,637 ms; low-level warm resume floor is 16–18 ms** | **100–200 ms required** |
...
3. Correct Assignment-time attribution and batch dispatch reduced unsaturated `runner_admission` from 26/43 ms to 17/31 ms p50/p95. Follow-up attribution isolated PostgreSQL execution as the claim cost, while pool acquisition, decode, and stream send remained 0 ms. Empty command and relay polls now avoid idle connection-row locks and writes. Eager Assignment dispatch then moved connection binding and control-sequence allocation into the existing serializable placement commit when the runner was idle, reducing a consecutive loaded-host comparison from 51/90 ms to 11/34 ms and the claim query from 33/65 ms to 0/1 ms. The regular durable claim remains the fallback under contention, Workspace creation, or reconnect. Re-run admission on a quiet qualified host and reduce p95 to at most 25 ms.
