...
| Pre-assignment orchestration | Latest 30-arrival measurement is 320 ms p50 and 488 ms p95, including 126/153 ms of Workspace provisioning | Must fit inside the total target |
| Command delivery and runner admission | Latest 30-arrival measurement is 17 ms p50 and 31 ms p95 | 10–25 ms required |
...
| **Total** | **Latest 30-arrival end-to-end is 745 ms p50 and 929 ms p95; low-level warm resume floor is 16–18 ms** | **100–200 ms required** |
...
3. Correct Assignment-time attribution and batch dispatch reduced unsaturated `runner_admission` from 26/43 ms to 17/31 ms p50/p95. The retained one-round-trip claim path measured queue at 6/15 ms and claim at 10/16 ms; stream send was 0/0 ms. Follow-up qualified attribution measured pool acquisition and decode at 0/0 ms while PostgreSQL execution consumed the entire claim duration at millisecond resolution. A pre-prepared statement regressed against one-round-trip execution on the same loaded host and was rejected. Empty command polls no longer lock or rewrite the active connection row, reducing an observed 11 idle tuple updates in two seconds to zero. The following empty relay poll now also avoids a transaction and connection-row lock, reducing 22 observed idle `xmax` changes in two seconds to zero while retaining an atomic fenced frame claim when work exists. Re-run admission on a quiet qualified host and reduce p95 to at most 25 ms.
