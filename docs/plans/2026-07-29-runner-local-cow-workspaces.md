...
- [ ] Change Snapshot creation and deletion from immediate `201`/`204` responses to `202 Operation` responses. Require `Idempotency-Key` for both, retain the parent Sandbox `If-Match` precondition for create, and deliberately omit `If-Match` from delete because Snapshot has no independent revision/ETag.
...
- [ ] Extend Operation with an optional Snapshot result/projection. Reserve and return the stable Snapshot ID in the create Operation from its first response, and populate the ready Snapshot on success so clients never have to discover a non-unique Snapshot by listing.
...
- [ ] Centralize the row-locking helper/order in the PostgreSQL store and add concurrent transaction tests for create/delete Snapshot, restore, start, every stop path, and Sandbox delete so no caller can invert Workspace/Snapshot acquisition or deadlock.
...
- [ ] Define a small private `WorkspaceStore` port for create/open, generation advance, Snapshot clone, restore stage/swap/finalize, Snapshot delete, workspace delete, inspect, and reconcile. Require operation IDs on every mutating method so retries return the same result.
...
- [ ] Implement a deterministic, traversal-safe directory layout beneath one required absolute `SECONDBOX_RUNNER_WORKSPACE_ROOT`, separating versioned workspace images, the current-image manifest, Snapshot images, staged images, rollback images, locks, and durable operation receipts.
...
- [ ] Implement restore preparation by reflinking a Snapshot into a new writable versioned image; implement the swap by fsyncing and atomically renaming a complete current-image manifest that names the new version and preserves the previous version as operation-scoped rollback state until finalize. Fsync the manifest's parent directory after rename and before recording or acknowledging the swap receipt.
- [ ] Implement idempotent local generation advance for ordinary stop. After compute is detached, atomically replace the current-image manifest with the same image reference and next generation, fsync its parent directory, then persist a replayable receipt before acknowledging stop completion.
- [ ] Persist runner-local receipts before acknowledging create, generation advance, delete, swap, or finalize. On restart, treat the current-image manifest plus receipts as authoritative evidence and safely finish or report interrupted operations.
...
- [ ] Add syscall abstraction unit tests plus real-filesystem qualification tests covering sparse creation, reflink-only behavior, copy-on-write isolation, atomic manifest publication, duplicate generation advance, duplicate operations, crashes at each rename/receipt boundary, locking, path validation, and disk-full errors.
...
- [ ] Add a provider-neutral local-workspace capability and versioned commands/results for workspace create/inspect/delete, generation advance, Snapshot create/delete, restore prepare/swap/finalize, and reconciliation.
...
- [ ] Make lifecycle admission lock and inspect the Workspace mutation slot. Acquire it for start before changing desired state and retain it through assignment/ready or terminal startup failure; a pending local mutation must prevent a conflicting start, stop, Snapshot, restore, or Sandbox delete from being accepted under the same stopped revision.
- [ ] Before the reconciler dispatches any compute stop, transactionally acquire or adopt the Workspace mutation slot under a stable stop effect ID. Cover explicit stop and drain Operations plus idle timeout, maximum duration, guest shutdown, guest-agent/liveness loss, runner recovery, deletion, and every other decision that can lead to `finish_stop` without a public Operation.
...
- [ ] Make ordinary stop flush and detach the active image, then execute the idempotent runner-local generation advance and return its durable receipt before `finish_stop` commits the matching next generation in PostgreSQL. Perform no Snapshot, hash, upload, retention update, or S3 request.
- [ ] Hold the durable Workspace mutation slot from the stop-producing lifecycle transition through detach, local generation-advance receipt, and the `finish_stop` database transaction. A failure before local advance may clear the slot transactionally; once an advance receipt exists, retain and retry the slot until PostgreSQL commits the matching generation.
- [ ] Reconcile a stop receipt when the runner advanced locally but the control-plane transaction did not commit; block start until PostgreSQL and the current-image manifest converge on the same generation.
- [ ] Make start resolve the current-image manifest after runner or control-plane restart and reject startup when the image is missing, already locked, wrong-sized, unformatted, or inconsistent with the expected generation.
...
- [ ] Add real Firecracker tests that write data, stop through explicit stop, drain, idle timeout, maximum duration, guest shutdown, and liveness-loss paths, restart on the same runner, verify generation/data/filesystem integrity, and prove no object-store request or image-sized I/O occurs.
...
- [ ] In one transaction, resolve idempotency, lock Sandbox/Workspace state, acquire the Workspace mutation slot, reserve the Snapshot ID/count, create the Operation with its Snapshot projection, and enqueue a durable runner effect without publishing the Snapshot as ready.
...
- [ ] Implement Snapshot delete as an async Operation: resolve IDs, lock Sandbox → Workspace → Snapshot in the global order, acquire the mutation slot, mark deletion intent transactionally, block new restore use, dispatch idempotent local deletion, and finalize metadata only after the runner receipt.
...
- [ ] Release the Workspace mutation slot in the same transaction that finalizes Snapshot create/delete. Prevent deletion of a Snapshot referenced by a pending or unfinalized restore, and define deterministic conflict handling for simultaneous create/delete/restore requests.
...
- [ ] In one transaction, resolve IDs, lock Sandbox → Workspace → Snapshot in the global order, and validate Snapshot ownership, ready state, stopped Sandbox, home-runner match, optimistic revision, no active writer, no conflicting Workspace mutation, and available home runner.
- [ ] In that admission transaction, advance the Sandbox revision, acquire the durable Workspace mutation slot, create the restore Operation, reserve the next generation, and enqueue a prepare effect without making the new generation current. Start, stop, Snapshot mutation, and Sandbox delete must observe the slot or new revision and cannot race the restore.
...
- [ ] After a prepare receipt, dispatch the fenced swap. The runner must atomically publish a current-image manifest naming the new version/generation, durably record the result, and retain the previous version as operation-scoped rollback state before acknowledging.
- [ ] Commit the new Sandbox generation, active-workspace evidence, audit entry, successful Operation, and mutation-slot release in PostgreSQL only after the durable swap receipt; advance the revision again for the observable completion.
...
- [ ] Add fault-injection tests at every receipt, manifest rename, dispatch, and database-commit boundary; add tests for stale revisions, wrong Sandbox, running Sandbox, start/stop/delete after restore admission, delete/restore race, multiple restore requests, offline/reconnecting runner, missing Snapshot file, and finalize retry.
