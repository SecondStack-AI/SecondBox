...
The `SecondBox Release Evidence` workflow is manually dispatched for one candidate version and commit. It has read-only repository and package permissions and contains no release, package-registry write, container-registry write, tag, or push operation. Its output is an Actions artifact for review, not a release. The evidence job is bound to the protected `release-evidence` environment because it consumes the release signing authority; that environment does not grant publication permissions.
...
The evidence dispatch requires a `candidate-run-id` from `.github/workflows/release-candidate.yml`. It downloads only the fixed `release-candidate-<commit>` artifact after querying the Actions API for the exact run and completed job. `scripts/verify-protected-release-workflow-run.mjs` requires the canonical repository, protected `main`, exact head SHA, `workflow_dispatch` event, successful `assemble-release-candidate` job, `secondbox-release` Runner group, required self-hosted labels, protected-environment identity, run ID, run attempt, and host name to agree. A different same-repository workflow or caller-authored identity fails.

The candidate artifact provides the exact local packages, three canonical digest-pinned GHCR references, and four source-and-digest-bound provenance statements. It also carries the GitHub environment API response captured inside the candidate job. `scripts/verify-protected-release-environment.mjs` requires a nonempty reviewer rule, self-review prevention, and a protected-branch-only deployment policy; naming an environment without configuring those protections fails. The signed guest archive is intentionally not copied through Actions artifact storage: the evidence job pulls the exact `guest-artifact-image` candidate digest, extracts its signed raw bundle, verifies its bundled key against `SECONDBOX_RELEASE_GUEST_TRUSTED_PUBLIC_KEY_SHA256`, reconstructs the deterministic archive, and requires its digest and size to equal the protected candidate record. This keeps the large guest payload content-addressed while still making the archive itself an exact signed release subject.

The evidence job imports those exact candidate bytes, emits the SBOM, vulnerability, dependency-age, license, checksum, signature, and provenance records, copies the canonical compatibility manifest, and records unavailable qualification dimensions as `blocked`. It never rebuilds or substitutes a candidate subject and never converts missing evidence into a skip or a passing empty record. A manual dispatch may additionally name a `qualification-run-id`; it is accepted only from the canonical `.github/workflows/release-qualification.yml` run and fixed `release-qualification-<commit>` artifact. Omitting the run leaves the qualification gates blocked.
...
The canonical qualification workflow itself requires all six structured records. It runs in the protected `release-qualification` environment on the `secondbox-release` Runner group with the `secondbox-release-qualification` label. It reconstructs the same protected candidate, verifies every record against that candidate, writes a workflow identity bound to the Actions run and actual Runner name, and captures and validates its environment configuration. Release evidence independently downloads the run and exact-attempt job API records, validates the captured environment response, and compares them with that identity before importing any qualification bytes. An arbitrary successful workflow run, feature-branch run, unprotected job or environment, other host group, changed attempt, or self-declared run ID is not qualification authority.
...
The protected `release-evidence` environment supplies `SECONDBOX_RELEASE_SIGNING_PRIVATE_KEY_PEM`; the workflow materializes it as an owner-only temporary file only for evidence generation and removes it on step exit. The repository or environment variable `SECONDBOX_RELEASE_TRUSTED_PUBLIC_KEY_SHA256` identifies the independently approved SHA-256 of the exact derived PEM public-key file. The generator and publication verifier compare that trust anchor before accepting the OpenSSL signature over `release-subjects.json`. The candidate and qualification workflows never receive this signing private key.
...
Configure these environments before producing a real candidate:

- `release-candidate`: require review, prevent self-review and administrator bypass, restrict deployments to protected `main`, and allow only the `secondbox-release` Runner group. Define `SECONDBOX_RELEASE_GUEST_SOURCE_DIRECTORY` as the canonical absolute raw signed-bundle directory on the release builder and `SECONDBOX_RELEASE_GUEST_TRUSTED_PUBLIC_KEY_SHA256` as its independently approved key fingerprint. This workflow may write only commit-and-run-scoped candidate manifests to the three GHCR packages; it cannot create Git tags, GitHub releases, versioned image tags, or npm versions.
- `release-qualification`: apply the same review and branch restrictions, allow only qualified hosts in the `secondbox-release` Runner group, define `SECONDBOX_RELEASE_QUALIFICATION_HOSTS_FILE` as the canonical qualified-host inventory, and define `SECONDBOX_RELEASE_QUALIFICATION_CONTROLLER_DIRECTORY` as the canonical directory of operator-reviewed scenario executables. Configure the explicit multi-Runner host, service, path, Profile, pool, timeout, API CA, and pinned SSH known-host variables listed in [qualification](qualification.md), and store the qualification API token, database URL, and SSH private key as environment secrets. The job reconstructs the candidate and executes these controllers; it does not import pre-authored non-multi-runner pass records.
- `release-evidence`: apply the same review and branch restrictions, store `SECONDBOX_RELEASE_SIGNING_PRIVATE_KEY_PEM` as an environment secret, and expose the approved guest and release public-key fingerprints as variables. The job has no publication write permission.

The Runner group and its labels are part of the verified run identity. Moving a job to a generic self-hosted Runner or supplying a successful run ID from another workflow does not satisfy the gate.

## Current blockers

The repository does not itself contain passing structured qualification records for packaged KVM execution, two-runner recovery, durability faults, the complete release-candidate data plane, network policy, or security boundaries. It also lacks adjacent released-version compatibility skew evidence. The protected workflows provide the only path to import the real registry subjects, approved guest bundle, external image/guest provenance, qualification evidence, and release signing authority; none are invented when those protected inputs are absent. The evidence workflow installs age-qualified Syft 1.44.0 and Grype 0.112.0 only after verifying their official checksum files and exact Linux-amd64 archive digests. Dependency-age generation covers Firecracker, kernel, Debian snapshot, and every resolved guest Python package. The canonical compatibility manifest records the relevant format and skew gaps.
