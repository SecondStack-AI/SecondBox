...
v0.7.0 carries the v0.6.0 Firecracker microVM bundle and trust anchor forward unchanged. Point `SECONDBOX_RUNNER_MICROVM_RELEASE_SOURCE_DIR` at the exact previously published signed bundle; do not rebuild it from the current experimental-backend guest sources. A different runtime or toolchain component-manifest digest makes the v1 guided updater reject v0.7.0 because existing Sandboxes remain pinned to their immutable Profile revisions. Microsandbox and gVisor use separate operator-local materializations that are not packaged by this release flow.
...
just release-candidate 0.7.0 /protected/releases/installer-candidate
...
just release-stage 0.7.0 /protected/releases/secondbox-0.7.0
just release-upload 0.7.0 /protected/releases/secondbox-0.7.0
```
