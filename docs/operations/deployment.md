...
`deployment.compose_backend_cidr` optionally assigns the Compose backend an explicit RFC1918 IPv4 `/24`. It must not overlap a same-host Runner's guest network. The guided installer always states this field after checking host routes and Docker IPAM allocations; omitting it preserves the behavior of older hand-authored manifests, including Docker's automatic network allocation.
