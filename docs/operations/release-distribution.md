...
The finalization workflow archives the resulting qualification attestation, constructs the acyclic index with the released deployment binary, publishes the qualification, and publishes the index as the last release asset. Only after the public index verifies does it promote the GitHub prerelease and npm `latest` distribution tag.
