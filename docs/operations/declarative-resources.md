...
A logical gateway mapping grants only DNS resolution and network access to that exact name and port. SecondBox does not assume that the destination is an HTTP proxy, inject proxy variables into Exec requests, or distribute an external proxy's interception CA. An application that supplies a forward proxy must pass its proxy environment explicitly and make the proxy CA available in the guest trust path. The application also owns CA rotation. This keeps external gateway authority out of immutable SecondBox runtime and toolchain bundles.
