...
The repository-owned qualification driver uses `qemu:///system` and creates three sequential, uniquely named Ubuntu guests. Each guest receives its own QEMU user network, deterministic MAC address, explicit NoCloud DHCP configuration, and verified localhost-only SSH forward; no host bridge, libvirt network, or firewall exception is required. The driver stores the pinned base-image copy and guest overlays beneath the explicit qualification workspace root, with traversal permissions for system libvirt; use a dedicated, capacious XFS or Btrfs mount that libvirt can traverse. Cleanup targets only that run's domains and disks and never uses the libvirt default network or an existing domain. Each guest receives nested KVM, a fresh root disk, and a separate data disk. One guest exercises the bounded Btrfs image, one performs a fresh candidate install on the explicit existing Btrfs filesystem, and one installs the pinned v0.6.0 release there before exercising the candidate's update decision.

v0.6.0 was published with an explicit installer-qualification waiver after two guest modes passed, so its unmodified deploy binary cannot verify its final manifest. For the update-source guest only, the driver builds a temporary adapter from the exact v0.6.0 source commit plus the checked-in narrow waiver-recognition patch. The adapter still downloads, digest-verifies, and installs the immutable public v0.6.0 manifest, binaries, bundles, evidence, and image references. It is never staged or published, and current releases remain subject to complete v2 installer evidence with successful reboot recovery.
