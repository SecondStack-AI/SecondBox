...
Back up the operation directory, Compose volumes, Runner identity, and the operation-specific Runner storage tree as one coordinated recovery point. That tree contains verified execution assets, Runner state, Workspaces, and local Snapshots on one reflink-capable filesystem. For the filesystem-image choice, the fully allocated Btrfs image is the complete durable Runner storage filesystem and must be captured while the deployment and Sandboxes are quiescent. Restoring PostgreSQL, S3 objects, or only the install receipt cannot reconstruct it. Permanent `uninstall --purge` is intentionally separate and is not a recovery mechanism. See [guided single-host installation](guided-single-host-install.md).
