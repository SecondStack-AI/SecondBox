...
func testSpec(pool string, cpu int64) secondboxclient.ProfileRevisionSpec {
	return secondboxclient.ProfileRevisionSpec{Pool: pool, Architecture: "amd64", RuntimeBundleDigest: "sha256:" + repeat("a", 64), ToolchainBundleDigest: "sha256:" + repeat("b", 64), Resources: secondboxclient.ResourcePolicy{CPUMillis: cpu, MemoryBytes: 1 << 30, WorkspaceBytes: 2 << 30, ProcessLimit: 64, ConcurrentOperations: 4}, Lifecycle: secondboxclient.LifecyclePolicy{InitialState: "running", DrainGraceSeconds: 10, IdleSeconds: 60, MaximumDurationSeconds: 900, LeaseSeconds: 60}, Retention: secondboxclient.RetentionPolicy{SnapshotLimit: 0, SnapshotRetentionSeconds: 1, ArtifactRetentionSeconds: 1}, Execution: secondboxclient.ExecutionPolicy{MaximumDeadlineMilliseconds: 1, MaximumBufferedOutputBytes: 1, StreamWindowBytes: 1, MaximumTransferBytes: 1, DataPlaneTransport: "proxied"}, Network: secondboxclient.NetworkPolicy{Mode: "deny_all", Destinations: []secondboxclient.NetworkDestination{}}, Ports: []secondboxclient.PortPolicy{}}
}
...
func TestApplyRejectsHistoricalDriftFutureHeadsGapsAndRevisionRaces(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"altered history", func(fake *fakeClient) { fake.profiles["profile"].Revisions[0].Spec.Resources.CPUMillis++ }},
