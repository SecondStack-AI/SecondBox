...
func TestPostgresTerminalProjectionOwnsAttachmentDetachAndFenceAuthority(t *testing.T) {
	...
	session, err = relay.StartDataPlaneSession(
	...
	if err != nil || recorded.InboundBytes != 4 {
		t.Fatalf("terminal output projection = %#v error=%v", recorded, err)
	}
	if detached, err := relay.DetachTerminalAttachment(
	...
	assertTerminalCancellation := func(sessionID string, expectedReason string) {
		t.Helper()
		var payload []byte
		...
		if cancel == nil ||
			cancel.Kind != runnerv1.DataPlaneSessionKind_DATA_PLANE_SESSION_KIND_PTY ||
			cancel.Reason != expectedReason {
			t.Fatalf("Terminal cancellation = %#v, want PTY reason %q", cancel, expectedReason)
		}
	}
	...
	assertTerminalCancellation("dps_terminal-lease-expired", "operation Lease is inactive")
	...
	assertTerminalCancellation(nonDetachable.ID, "public Terminal client disconnected")
}
