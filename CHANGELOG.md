...
- Guided installation now inspects existing Docker IPAM allocations and chooses a separate, reviewed RFC1918 `/24` for the Compose backend network. Compose startup no longer depends on Docker's finite automatic address pools, and both the Firecracker guest network and Compose network are rejected when they overlap each other, a host route, or an allocated Docker network.
