...
- Added `SECONDBOX_RUNNER_GATEWAYS` to the guest execution environment, publishing each logical gateway the Sandbox's pinned egress context resolved as a sorted `logicalName=address:port` entry, so applications no longer configure a Runner-host address themselves.
...
- The runner protocol advances to generation 3 exactly, carrying provider-neutral execution-asset identities and backend materialization evidence. Generation 1 and 2 runners are refused. Upgrade as one coordinated rollout: stop all Runners, upgrade and start the control plane (which applies migration `0019`), then deploy and start the upgraded Runners. A mixed-version fleet is not supported ([#96](https://github.com/SecondStack-AI/SecondBox/pull/96)).
