...
func TestInitSingleHostFromReleaseMaterializesEveryAcceptedRunnerValue(t *testing.T) {
	...
	if runner.RunnerID != result.RunnerID || runner.IdentityHostDirectory != result.RunnerIdentityDirectory || runner.WorkspaceHostDirectory != plan.Storage.WorkspacePath || runner.ArtifactHostDirectory != installPath(plan, "artifacts") || runner.StateHostDirectory != installPath(plan, "runner-storage") || runner.FirecrackerJailerUIDStart == nil || *runner.FirecrackerJailerUIDStart != plan.Network.JailerUIDRange.Start || runner.MaxConcurrentOperationsGlobal == nil || *runner.MaxConcurrentOperationsGlobal != plan.Capacity.ConcurrentOperations || *runner.MaxConcurrentOperationsGlobal < install.DurableCodingConcurrentOperations || runner.NetworkPolicyDNSUpstream != "[2001:db8::53]:53" || runner.DataPlaneAdvertisedAddress != plan.Network.DataPlaneAddress || runner.ArtifactPublicKeySHA256 != keyID {
