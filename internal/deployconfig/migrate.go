...
func MigrateLegacyEnvironment(sourcePath, targetDirectory string) (string, error) {
	...
	if mode != "development" {
		return "", manifestError("legacy production migration cannot infer standard_resources; provide a reviewed production manifest", nil)
	}
	...
	if err := writeAtomic(filepath.Join(absoluteTarget, "development-artifact-manifest.json"), append(releaseManifest, '\n'), 0o644, false); err != nil {
		return "", err
	}
...
func legacyEnvironmentNames() map[string]bool {
	names := []string{
		...
		"SECONDBOX_GARBAGE_COLLECTION_POLL_INTERVAL_MILLISECONDS",
