...
func singleHostManifest(plan install.InstallPlan, release releasecontract.ArtifactManifest, signingKeyID, runnerID, runnerIdentity, postgresPassword, platformToken, runnerCredential, catalogPath, releasePath, pkiPath string) (ManifestV1, error) {
	...
	pools := []StandardRunnerPool{{Name: standardresources.PoolAMD64, Architectures: []string{"amd64"}, Capabilities: slices.Clone(features), State: "ready", MaxSandboxes: integer(plan.Capacity.MaxSandboxes), MaxVCPUCount: integer(plan.Capacity.VCPUCount()), MaxMemoryBytes: integer(plan.Capacity.MaxMemoryBytes)}}
	...
	contextGateways := make([]RunnerLogicalGateway, 0, len(gatewayEntries))
	for _, entry := range gatewayEntries {
		logicalName, address, _ := strings.Cut(entry, "=")
		contextGateways = append(contextGateways, RunnerLogicalGateway{LogicalName: logicalName, Address: address})
	}
	contextName := "secondbox-" + strings.ReplaceAll(strings.TrimPrefix(plan.OperationID, "install_"), "_", "-")
