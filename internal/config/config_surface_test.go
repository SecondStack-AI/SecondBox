...
func TestFromEnvironmentUsesLiteralTuningDefaultsAndValidatedOverrides(t *testing.T) {
	...
	if got.HTTPTimeout != 30*time.Second || got.RunnerHeartbeatInterval != 5000*time.Millisecond ||
		...
		got.DataPlanePollInterval != 250*time.Millisecond || got.RunnerCommandPollInterval != 250*time.Millisecond ||
...
func TestFromEnvironmentPollingOverridesKeepMillisecondUnits(t *testing.T) {
	setRequiredControlPlaneEnvironment(t)
	t.Setenv("SECONDBOX_RUNNER_COMMAND_POLL_INTERVAL_MILLISECONDS", "17")
	t.Setenv("SECONDBOX_DATA_PLANE_POLL_INTERVAL_MILLISECONDS", "31")
	got, err := FromEnvironment()
	if err != nil {
		t.Fatal(err)
	}
	if got.RunnerCommandPollInterval != 17*time.Millisecond || got.DataPlanePollInterval != 31*time.Millisecond {
		t.Fatalf("polling overrides: command=%s data-plane=%s", got.RunnerCommandPollInterval, got.DataPlanePollInterval)
	}
	for _, name := range []string{"SECONDBOX_RUNNER_COMMAND_POLL_INTERVAL_MILLISECONDS", "SECONDBOX_DATA_PLANE_POLL_INTERVAL_MILLISECONDS"} {
		t.Run(name, func(t *testing.T) {
			t.Setenv(name, "0")
			if _, err := FromEnvironment(); err == nil || !strings.Contains(err.Error(), name) {
				t.Fatalf("zero polling interval error = %v", err)
			}
		})
	}
}
...
func TestFromEnvironmentRejectsEveryUnusableTuningOverride(t *testing.T) {
	if len(TuningDefaults()) != 17 {
		t.Fatalf("tuning surface = %d, want 17", len(TuningDefaults()))
	}
	for _, tuning := range TuningDefaults() {
