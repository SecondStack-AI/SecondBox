...
func TestLegacyCapacitySpellingRoundTripsWithStableIdentity(t *testing.T) {
	...
	plan.Capacity.LegacySubjectQuotas = map[string]int64{"maxSandboxes": 20, "maxActiveInstances": 8, "maxMemoryBytes": 24 << 30, "maxSnapshots": 100, "maxPortSessions": 50, "maxConcurrentOperations": 8}
	plan.Capacity.LegacySubjectQuotas["maxCpuMillis"] = 4500
	...
	if got := decoded.Capacity.LegacySubjectQuotas["maxCpuMillis"]; got != 4500 {
		t.Fatalf("recorded legacy Subject CPU quota = %d, want 4500 milli-units", got)
	}
...
func TestFrozenV060PlanDecodesWithStableIdentity(t *testing.T) {
	...
	if got := plan.Capacity.LegacySubjectQuotas["maxCpuMillis"]; got != 32000 {
		t.Fatalf("frozen plan Subject CPU quota = %d, want 32000 milli-units", got)
	}
