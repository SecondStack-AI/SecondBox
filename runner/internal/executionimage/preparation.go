...
// PrepareThroughFetcher is every backend's Runner-side preparation. Staging
// capacity is admitted by the fetcher against the cache filesystem it writes
// to, and only when the digest is absent. The Runner cannot admit it here: the
// request carries a tag, so warm and cold are indistinguishable until the
// fetcher has resolved the digest.
func PrepareThroughFetcher(
	ctx context.Context,
	socket string,
	...
	result, err := FetchExecutionImage(ctx, socket, input, func(result FetchResult) error {
