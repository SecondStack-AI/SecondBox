...
func relocateRunDirForUnixSockets(runDir string) (string, bool) {
	...
	return filepath.Join(os.TempDir(), fmt.Sprintf("secondbox-runner-%d", os.Getuid()), "run"), true
}

func shortMicroVMRunDirCandidates() []string {
	var candidates []string
	if cacheDir, err := os.UserCacheDir(); err == nil && strings.TrimSpace(cacheDir) != "" {
		candidates = append(candidates, filepath.Join(cacheDir, "secondbox-runner", "run"))
	}
	candidates = append(candidates, filepath.Join(os.TempDir(), fmt.Sprintf("secondbox-runner-%d", os.Getuid()), "run"))
	return candidates
}
...
func captureTrustedMicroVMArtifacts(cfg *config.Config) (*trustedMicroVMArtifacts, error) {
	...
	paths := []trustedMicroVMArtifactFile{
		...
		{label: "rootfs contract", path: filepath.Join(artifactDir, "secondbox-rootfs-contract.json")},
		{label: "Debian package lock", path: filepath.Join(artifactDir, "rootfs-debian-packages.lock")},
		{label: "Python freeze", path: filepath.Join(artifactDir, "rootfs-python.freeze")},
		{label: "Debian license inventory", path: filepath.Join(artifactDir, "rootfs-debian-license-inventory.json")},
		{label: "Python license inventory", path: filepath.Join(artifactDir, "rootfs-python-license-inventory.json")},
		{label: "runtime component manifest", path: filepath.Join(artifactDir, "runtime-manifest.json")},
		{label: "toolchain component manifest", path: filepath.Join(artifactDir, "toolchain-manifest.json")},
...
func newInstanceID(sandboxID, compartmentID string) (string, error) {
	var b [4]byte
	if _, err := rand.Read(b[:]); err != nil {
		return "", fmt.Errorf("generate instance id: %w", err)
	}
	return instancePrefix + "-" + instanceSandboxIDSegment(sandboxID) + "-" + compartmentIDSegment(compartmentID) + "-" + hex.EncodeToString(b[:]), nil
}

func instanceSandboxIDSegment(sandboxID string) string {
...
func reflinkOnlyFile(dst, src string, mode os.FileMode) error {
	...
	if err := reflinkFile(out, in); err != nil {
		...
		if errors.Is(err, syscall.EXDEV) {
			hint = "dst and src are on different filesystems; set SECONDBOX_RUNNER_FIRECRACKER_RUN_DIR to a path on the same filesystem as SECONDBOX_RUNNER_FIRECRACKER_ROOTFS_PATH"
		}
...
func (m *Manager) guestProtocolBootArgs(instanceID, sandboxID string, opts runtimemanager.StartOpts) (string, error) {
	values := map[string]string{
		...
		"secondbox.sandbox_id":                sandboxID,
		"secondbox.sandbox_generation":        strconv.FormatUint(opts.SandboxGeneration, 10),
		"secondbox.guest_build_id":            opts.GuestBuildID,
		"secondbox.image_manifest_digest":     opts.ImageManifestDigest,
		"secondbox.toolchain_manifest_digest": opts.ToolchainManifestDigest,
		...
		"secondbox.guest_heartbeat_interval":  m.cfg.MicroVMGuestHeartbeatInterval.String(),
	}
	if opts.SandboxGeneration == 0 ||
		m.cfg.MicroVMGuestControlVsockPort == 0 ||
		m.cfg.MicroVMGuestProtocolVsockPort == 0 ||
		m.cfg.MicroVMGuestControlVsockPort == m.cfg.MicroVMGuestProtocolVsockPort ||
		m.cfg.MicroVMGuestHeartbeatInterval <= 0 {
		return "", fmt.Errorf("guest protocol launch identity and dedicated vsock configuration are incomplete")
	}
	order := []string{
	...
	for _, name := range order {
		...
		if value == "" || strings.ContainsAny(value, " \t\r\n=") {
			return "", fmt.Errorf("guest protocol boot value %s is missing or unsafe", name)
		}
...
func (m *Manager) writeIdentityFile(dir, instanceID, sandboxID string, opts runtimemanager.StartOpts) error {
...
// control service so startup secrets can be delivered before the runtime starts.
const controlPlaneReadyTimeout = 60 * time.Second

// buildStartupSecretBundle assembles the assignment identity delivered over vsock.
func (m *Manager) buildStartupSecretBundle(sandboxID, instanceID string, opts runtimemanager.StartOpts) (SecretBundle, error) {
	env := map[string]string{
		"SECONDBOX_SANDBOX_ID":     sandboxID,
		"SECONDBOX_INSTANCE_ID":    normalizeRuntimeCompartmentID(opts.CompartmentID),
		"SECONDBOX_GUEST_HOST_GID": strconv.Itoa(os.Getgid()),
		"TZ":                       normalizeTimezone(opts.Timezone),
	}
	if strings.TrimSpace(instanceID) != "" {
		env["SECONDBOX_RUNTIME_CREDENTIAL_ID"] = sandboxID + ":" + normalizeRuntimeCompartmentID(opts.CompartmentID) + ":" + strings.TrimSpace(instanceID)
	}
