...
func TestRelocateRunDirForUnixSockets(t *testing.T) {
	...
	t.Run("deep path relocates under cache dir", func(t *testing.T) {
		...
		deep := "/home/sasha/Developer/tries/secondbox-runner/data/microvm/run"
	...
	t.Run("default user cache falls back when a full instance suffix would overflow", func(t *testing.T) {
		...
		want := filepath.Join(os.TempDir(), fmt.Sprintf("secondbox-runner-%d", os.Getuid()), "run")
		if got != want {
...
func TestCheckUnixSocketPath(t *testing.T) {
	tests := []struct {
		...
		{name: "fits", path: "/run/user/1000/secondbox-runner/run/fc-abc-cmp-123-4567/firecracker.sock", wantErr: false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := checkUnixSocketPath("test", tt.path, "SECONDBOX_RUNNER_MICROVM_RUN_DIR")
