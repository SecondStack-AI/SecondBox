...
func (a *thinDeviceIDAllocator) lookup(seed string) (uint32, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	if err := a.loadLocked(); err != nil {
		return 0, err
	}
	id, ok := a.store.IDs[seed]
	if !ok {
		return 0, fmt.Errorf("dm-thin device id for %q is unavailable", seed)
	}
	return id, nil
}
...
func (m *Manager) ensureThinWorkspace(ctx context.Context, sandboxID, compartmentID string) (string, error) {
	return m.ensureThinWorkspaceForAttachment(ctx, sandboxID, compartmentID, compartmentID)
}

func (m *Manager) removeReleasedWorkspace(ctx context.Context, sandboxID, attachmentID string) error {
	if strings.EqualFold(m.cfg.MicroVMWorkspaceBackend, "dm-thin") {
		name := thinWorkspaceName(sandboxID, attachmentID)
		deviceID, err := m.thinDeviceIDAllocatorRef().lookup("workspace-origin:" + name)
		if err != nil {
			return fmt.Errorf("lookup released dm-thin workspace: %w", err)
		}
		if output, err := runHostCommand(ctx, "dmsetup", "remove", name); err != nil {
			return fmt.Errorf("remove released dm-thin mapping: %w: %s", err, strings.TrimSpace(string(output)))
		}
		if output, err := runHostCommand(
			ctx,
			"dmsetup",
			"message",
			m.cfg.MicroVMThinPoolDevice,
			"0",
			fmt.Sprintf("delete %d", deviceID),
		); err != nil {
			return fmt.Errorf("delete released dm-thin device: %w: %s", err, strings.TrimSpace(string(output)))
		}
		return nil
	}
	path := filepath.Join(m.cfg.MicroVMWorkspaceDir, sandboxID, attachmentID+"."+workspaceName)
	if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) {
		return fmt.Errorf("remove released ext4 workspace %q: %w", path, err)
	}
	return nil
}

func (m *Manager) ensureThinWorkspaceForAttachment(ctx context.Context, sandboxID, compartmentID, attachmentID string) (string, error) {
	...
	if _, err := runHostCommand(ctx, "dmsetup", "info", name); err == nil {
		...
		if err := m.ensureThinWorkspaceFilesystem(ctx, path, sandboxID, compartmentID); err != nil {
			return "", err
		}
		return path, nil
	}
...
func (m *Manager) ensureThinWorkspaceFilesystem(ctx context.Context, path, sandboxID, compartmentID string) error {
...
func (m *Manager) CreateWorkspaceThinSnapshot(ctx context.Context, instanceID, snapshotName string) (_ ThinWorkspaceSnapshot, resultErr error) {
	if !strings.EqualFold(m.cfg.MicroVMWorkspaceBackend, "dm-thin") {
	...
	defer func() {
		_, thawErr := m.ThawWorkspace(context.Background(), instanceID)
		resultErr = errors.Join(resultErr, thawErr)
	}()
	origin := thinWorkspaceName(inst.sandboxID, firstNonEmpty(inst.workspaceAttachmentID, inst.compartmentID))
...
func thinWorkspaceName(sandboxID, compartmentID string) string {
	name := thinNameUnsafe.ReplaceAllString(strings.TrimSpace(sandboxID), "-")
