...
func validateRunnerGateways(gateways []networkpolicy.LogicalGatewayEndpoint) error {
	for _, gateway := range gateways {
		...
		if !address.Is4() || !(address.IsGlobalUnicast() || address.IsLinkLocalUnicast()) || gateway.Endpoint.Port() == 0 {
			return fmt.Errorf("Runner gateway %q requires a unicast IPv4 endpoint with a nonzero port", gateway.LogicalName)
		}
	}
	return nil
}
