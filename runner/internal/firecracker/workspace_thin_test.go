...
func TestEnsureThinWorkspaceCreatesAndFormatsDevice(t *testing.T) {
	...
	if path != "/dev/mapper/secondbox-ws-agent-1-cmp_a" {
		t.Fatalf("path = %q", path)
	}
	joined := strings.Join(calls, "\n")
	for _, want := range []string{
		"dmsetup info secondbox-ws-agent-1-cmp_a",
		"dmsetup message /dev/mapper/secondbox-pool 0 create_thin ",
		"dmsetup create secondbox-ws-agent-1-cmp_a --table 0 2097152 thin /dev/mapper/secondbox-pool ",
		"blkid -o value -s TYPE /dev/mapper/secondbox-ws-agent-1-cmp_a",
		"mkfs.ext4 -F -q -E lazy_itable_init=1,lazy_journal_init=1,nodiscard /dev/mapper/secondbox-ws-agent-1-cmp_a",
	} {
...
func TestEnsureThinWorkspaceFormatsActiveDeviceLeftUnformattedByCrash(t *testing.T) {
	...
	m := &Manager{cfg: &config.Config{MicroVMThinPoolDevice: "/dev/mapper/secondbox-pool", MicroVMWorkspaceSizeMiB: 1024}}
	if _, err := m.ensureThinWorkspace(context.Background(), "agent-crash", "cmp_a"); err != nil {
		t.Fatal(err)
	}
	joined := strings.Join(calls, "\n")
	if !strings.Contains(joined, "blkid -o value -s TYPE /dev/mapper/secondbox-ws-agent-crash-cmp_a") ||
...
func TestEnsureThinWorkspaceSeedsNewDeviceFromCompartmentWorkspace(t *testing.T) {
	dataDir := t.TempDir()
	defaultSeedDir := filepath.Join(dataDir, "sandboxes", "agent-seeded", "workspace")
	if err := os.MkdirAll(defaultSeedDir, 0o700); err != nil {
		t.Fatal(err)
	}
	seedDir := filepath.Join(dataDir, "sandboxes", "agent-seeded", "compartments", "cmp_a", "workspace")
...
func TestCreateThinSnapshotCommands(t *testing.T) {
	...
	snap, err := m.createThinSnapshot(context.Background(), "secondbox-ws-agent-1", "backup:daily")
	if err != nil {
		t.Fatalf("create thin snapshot: %v", err)
	}
	if snap.DevicePath != "/dev/mapper/secondbox-ws-agent-1-snap-backup-daily" {
		t.Fatalf("snapshot = %#v", snap)
	}
	joined := strings.Join(calls, "\n")
	for _, want := range []string{
		"dmsetup message /dev/mapper/secondbox-pool 0 create_snap ",
		"dmsetup create secondbox-ws-agent-1-snap-backup-daily --table 0 1048576 thin /dev/mapper/secondbox-pool ",
	} {
		if !strings.Contains(joined, want) {
			t.Fatalf("calls missing %q:\n%s", want, joined)
		}
...
func assertThinIDsInRange(t *testing.T, commands string) {
	t.Helper()
	matches := regexp.MustCompile(`(?:create_thin|create_snap|thin /dev/mapper/secondbox-pool) (\d+)`).FindAllStringSubmatch(commands, -1)
	if len(matches) == 0 {
