...
require_env SECONDBOX_RUNNER_FIRECRACKER_JAILER_UID
...
require_env SECONDBOX_RUNNER_SANDBOX_STORAGE_BACKEND
require_env SECONDBOX_RUNNER_CHECKPOINT_RESTORE_SPOOL_DIR
...
case "$SECONDBOX_RUNNER_SANDBOX_STORAGE_BACKEND" in
    ext4) ;;
    dm-thin)
        require_env SECONDBOX_RUNNER_SANDBOX_THIN_POOL_DEVICE
        [ -e "$SECONDBOX_RUNNER_SANDBOX_THIN_POOL_DEVICE" ] || fail "SECONDBOX_RUNNER_SANDBOX_THIN_POOL_DEVICE not found: $SECONDBOX_RUNNER_SANDBOX_THIN_POOL_DEVICE"
        command -v dmsetup >/dev/null 2>&1 || fail "dmsetup is required for dm-thin workspaces"
        ;;
    *) fail "SECONDBOX_RUNNER_SANDBOX_STORAGE_BACKEND must be ext4 or dm-thin" ;;
esac
