...
if "$script_dir/verify.sh" "$work_dir" >"$work_dir/out" 2>"$work_dir/err"; then
    echo "verify accepted an artifact set without a trusted public key" >&2
    exit 1
fi
grep -q 'trusted public key is required' "$work_dir/err"

if "$script_dir/verify.sh" "$work_dir" "$work_dir/public.pem" >"$work_dir/out" 2>"$work_dir/err"; then
    echo "verify accepted a trusted public key without its fingerprint" >&2
    exit 1
fi
grep -q 'trusted public key fingerprint is required' "$work_dir/err"
