...
template_mode="$(kernel_arg secondbox.template_mode || true)"
...
# A snapshot-resume template boots with no Sandbox identity at all. It serves only
# the host-only control endpoint; identity arrives through one /assignment/bind
# request after /restore/harden succeeds, and only then does the guest protocol
# listener accept.
